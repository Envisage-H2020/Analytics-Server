...
def WriteToCsv():
    windBashCommand = "mongoexport --db envisage --collection windUser --type csv --fields _id,state.environment,state.map,state.turbine,state.binary,results.profit,changedSimSpeed,correctPower,results.correct,results.medianTimeOnTask,results.over,results.under,results.score,repairedTurbine,turbineOnOff,results.name,userId --out windUser.csv"
